...
func NewCmdCreate(f *factory.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  devicemanager tunnel create --name ssh-tunnel --device-id <id> --proto tcp --local-address 127.0.0.1 --local-port 22`,
	...
	cmd.Flags().StringVar(&opts.LocalAddress, "local-address", "127.0.0.1", "Device-side local address to forward")
	cmd.Flags().IntVar(&opts.LocalPort, "local-port", 0, "Device-side local port to forward (required)")
