...
func NewCmdApi(f *factory.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Make an authenticated API request to the DM platform.
...
Authorization header is automatically injected.`,
  ...
  elements api /api/devices/DEVICE_ID/files/capture_result.pcap --output-file capture.pcap`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Format output based on -o flag
			output, _ := cmd.Flags().GetString("output")
			if body != nil {
				if fmtErr := iostreams.FormatOutput(body, f.IO, output, iostreams.WithColumns(opts.Columns...)); fmtErr != nil {
					return fmtErr
				}
			}
