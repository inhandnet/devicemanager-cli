...
func newCmdConfigDevices(f *factory.Factory) *cobra.Command {
	opts := &cmdutil.ListFlags{}

	cmd := &cobra.Command{
		Use:     "devices <app-id>",
		Short:   "List devices with an edge app config deployed",
		Aliases: []string{"ls"},
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			body, err := client.Get(fmt.Sprintf("/api/edge/apps/%s/configs/devices", args[0]), q)
			if err != nil {
				return err
			}
